...
cygconf() {
	...
	${S}/configure \
		...
		--exec-prefix=/usr \
		--bindir=/usr/bin \
		--includedir=/usr/include/freetds \
		--libdir=/usr/lib \
		--libexecdir=/usr/sbin \
		--sbindir=/usr/sbin \
		--datadir=/usr/share \
...
cygdoc() {
	...
	cd ${S}
	mkdir -p ${cygdocdir} ${pkghtmldir}

	mv ${pkgdocdir}/images ${pkgdocdir}/reference ${pkgdocdir}/userguide ${pkghtmldir}
...
cygpkg() {
	...
	echo
	for l in ${LIB_PKGS} ; do
		for n in 2 3 4 5 6 7 8 9 ; do
			if [ -f usr/bin/cyg$l-$n.dll ] ; then
				tar cvjf ${T}/lib$l$n-${PV}-${PR}.tar.bz2 usr/bin/cyg$l-$n.dll
				echo
				break
			fi
		done
...
dist() {
	...
	for l in ${LIB_PKGS} ; do
		for n in 2 3 4 5 6 7 8 9 ; do
			if [ -f ${T}/lib$l$n-${PV}-${PR}.tar.bz2 ] ; then
				mkdir -p ${T}/${PN1}/lib$l$n
				mv ${T}/lib$l$n-${PV}-${PR}.tar.bz2 ${T}/${PN1}/lib$l$n/
				cp ${C}/lib$l.hint ${T}/${PN1}/lib$l$n/setup.hint
				break
			fi
		done
	done
